...
Item {
    ...
    // Item construction moved to the ScriptureItems singleton so narration
    // builds identical items (see ScriptureItems.qml for why). These wrappers
    // keep every call site in this file unchanged and supply the tab's active
    // translation as the attribution fallback.
    function buildItemFromVerse(verse) {
        return ScriptureItems.fromVerse(verse, activeTranslation)
    }

    function buildItemFromVerses(verses) {
        return ScriptureItems.fromVerses(verses, activeTranslation)
    }

    function _formatCopyText(verses) {
        return ScriptureItems.copyText(verses, activeTranslation)
    }

    function _formatVerseRangeTitle(verses) {
        return ScriptureItems.rangeTitle(verses)
    }
